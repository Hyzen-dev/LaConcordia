...
export default function EventsCreate() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (newErrors.length > 0) {
      ...
      const response = await useApi.events.Create({ title: title, content: richContent, thumbnail: medias })
    ...
    <div className='usersPage'>
      ...
      <div className='usersPage__content'>
        <form onSubmit={(event) => handleSubmit(event)} className='form createForm'>
          <label htmlFor="title" className='usersPage__subheading'>Titre de l'évènement</label>
          <input value={title} onChange={(event) => setTitle(event.currentTarget.value)} type="text" name='title' placeholder='Ajouter le titre' />
          <label htmlFor="content" className='usersPage__subheading'>Contenu de l'évènement</label>
